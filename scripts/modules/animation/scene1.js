...
export const scene1 = async ({ foreground, middleground, background }) => {
	...
	await typeDialog(narrator, 'Julenissen har endelig begynt på sin lange reise verden rundt, for å levere julegavene i god tid før julaften.');
	await typeDialog(narrator, 'Som regel så er det en blanding av dårlig vær og sikte, eller et og annet utilregnelig flygende objekt som er den største tidstyven for julenissen');
	await typeDialog(narrator, 'Men i år, i en liten bygd i Norge, er det faktisk noe litt annet som vil sette kjepper i reinsdyrene på hans vei...');
